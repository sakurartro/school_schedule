...
def schedule_table(day_lessons) -> InputRichBlockTable:
    cells = [
        [make_cell("Время", True), make_cell("Урок", True), make_cell("Кабинет", True)]
    ]
...
async def sent_schedule_tomorrow(message: Message, state: FSMContext) -> None:
...
async def scheduled_msg(bot: Bot) -> None:
    ...
    for data in await get_all_data():
        ...
        if schedule2 is None:
            ...
            all_lessons = [schedule, schedule2]
            days_row = []
            header_row = []
            for day in all_lessons:
                label = f"📍 {day.weekday}"
                days_row.append(make_cell(label, True, colspan=3))
                header_row.extend(
                    [
                        make_cell("Время", True),
                        make_cell("Урок", True),
            ...
            cells = [days_row, header_row]

            max_lessons = max((len(day.lessons) for day in all_lessons))
            ...
            rich_data = InputRichBlockTable(is_bordered=True, is_striped=True, cells=cells)
            rich_msg = InputRichMessage(
                blocks=[
                    emoji_heading("на ближайшие дни", "bell_gold_ribbon_1"),
                    rich_data,
                ]
            )
            await bot.send_rich_message(
